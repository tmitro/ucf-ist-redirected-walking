...
public class SimulatedWalker : MonoBehaviour {
	...
	void Start()
    {
        redirectionManager = GameObject.Find("Redirected User").GetComponent<RedirectionManager>();
    }
    ...
    public void WalkIfPossible(float rotationToTargetInDegrees, Vector3 userToTargetVectorFlat)
    {
        ...
        if (Mathf.Abs(rotationToTargetInDegrees) < ROTATIONAL_ERROR_ACCEPTED_IN_DEGRESS)
        {
            ...
            float distanceToTravel = Mathf.Min(Mathf.Min(redirectionManager.GetDeltaTime() * translationSpeed, userToTargetVectorFlat.magnitude), 0.001f + redirectionManager.resetter.getMaxWalkableDistanceBeforeReset());
